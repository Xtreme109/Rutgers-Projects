...
public class GoHomeMachine {
    ...
    public static void main(String[] args) {
        ...
        Spiderverse graph = new Spiderverse();

        Dimension source = new Dimension(0, 4, 3);

        for (Integer i : adjlist.keySet()){
            ArrayList<Dimension> curr = adjlist.get(i);
        ...
        graph = graph.Dijkstra(graph, source);
        Set<Dimension> edges = graph.getNodes();
        ...
        for (int i = 0; i < num_anomalies; i++) {
            ...
            Person anomaly = sv.getPerson(name);
            int return_dimension = anomaly.getSignature();
            
            for (Dimension dimension : edges) {
                if (dimension.getNumber() == return_dimension){

                    if (time > dimension.getTimetoTravelPath()) {
