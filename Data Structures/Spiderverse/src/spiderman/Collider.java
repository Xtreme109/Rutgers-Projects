...
public class Collider {
    ...
    public static void main(String[] args) {
        ...
        StdOut.setFile(args[2]); // collider.out

        Hashtable<Integer, ArrayList<Dimension>> adjlist = sv.createAdjacencyList();

        for (Map.Entry<Integer, ArrayList<Dimension>> entry : adjlist.entrySet()) {
            ArrayList<Dimension> values = entry.getValue();

            // Iterate through the ArrayList for this key
            for (Dimension d : values) {
                StdOut.print(d.getNumber() + " ");
            }
            StdOut.println();
        }
