...
public class RUHungry {
    ...
    public void menu(String inputFile) {
	    ...
	    int categAmount = StdIn.readInt();
        categoryVar = new String[categAmount];
        menuVar = new MenuNode[categAmount];
        for (int i = 0; i < categAmount; i++) {
            String categName = StdIn.readString();
            categoryVar[i] = categName;
            int menuAmount = StdIn.readInt();

            MenuNode first = null;
            for (int j = 0; j < menuAmount; j++) {
                StdIn.readLine();
                String dishName = StdIn.readLine();
                int ingredientAmount = StdIn.readInt();

                int[] tempid = new int[ingredientAmount];
                for (int k = 0; k < ingredientAmount; k++) {
                    int ingredientid = StdIn.readInt();
                    tempid[k] = ingredientid;
                }
                Dish a = new Dish(categName,dishName,tempid);
                
                MenuNode currentNode = new MenuNode(a, first);
                menuVar[i] = currentNode;   //so the last node is the start when loop is finished
                first = currentNode;
            }
        }
    ...
    public void addStockNode (StockNode newNode) {
        ...
        int newid = newNode.getIngredient().getID();
        int hash = newid % stockVarSize;

        
        newNode.setNextStockNode(stockVar[hash]);
        stockVar[hash] = newNode;   //so the last node is the start when loop is finished  
    }
    ...
    public StockNode findStockNode (int ingredientID) {
        ...
        return a; // update the return value
    }
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        if (ingredientName != null) {
            a = findStockNode(ingredientID);
        } else if (ingredientID != -1) {
            a = findStockNode(ingredientName);
        }

        int stockLvl = a.getIngredient().getStockLevel();
        a.getIngredient().setStockLevel(stockLvl+stockAmountToAdd);
    }
    ...
    public void updatePriceAndProfit() {
        ...
        for (int i = 0; i < menuVar.length; i++) {
            ...
            while (ptr != null) {
                // System.out.println("Dish: " + ptr.getDish().getDishName());
                int[] ptrstock = ptr.getDish().getStockID();
                double ingredientsCost = 0;
                double dishprofit = 0;
                for (int j = 0; j < ptrstock.length; j++) {
                    StockNode temp = findStockNode(ptrstock[j]);
                    // System.out.println("Ingredient: " + temp.getIngredient().getName());
                    ingredientsCost = temp.getIngredient().getCost() + ingredientsCost;
                }


                ptr.getDish().setPriceOfDish(ingredientsCost*1.2);
                double trueprice = ptr.getDish().getPriceOfDish();
                dishprofit = trueprice - ingredientsCost;
                ptr.getDish().setProfit(dishprofit);
    ...
    public void createStockHashTable (String inputFile){
        ...
        int newSize = StdIn.readInt();
        stockVarSize = newSize;
        ...
        while (StdIn.readLine() != null) {
            ...
            StdIn.readChar();
            String ingredName = StdIn.readLine();
            Double ingredCost = StdIn.readDouble();
            int ingredAmount = StdIn.readInt();


            Ingredient newIngred = new Ingredient(id, ingredName, ingredAmount, ingredCost);
            StockNode newNode = new StockNode(newIngred, null);
    ...
    public void addTransactionNode ( TransactionData data ) { // method adds new transactionNode to the end of LL
        ...
        TransactionNode current = new TransactionNode(data, null);
        TransactionNode ptr = transactionVar;
        if(transactionVar == null) { 
           transactionVar = current;
        } else { 
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        ...
        int[] resources = current.getDish().getStockID();

        for (int i = 0; i < resources.length; i++) {
            StockNode temp = findStockNode(resources[i]);
            int stock = temp.getIngredient().getStockLevel();
    ...
    public void order (String dishName, int quantity){
        ...
        if(check == true){ 
            successfulTrans(dishName, quantity);
        } else{ 
            unsuccessfulTrans(dishName, quantity); 
        }
        
    }

    private void successfulTrans(String dishName, int quantity) { 
        Double profit = findDish(dishName).getDish().getProfit() * quantity;
        TransactionData worked = new TransactionData("order", dishName, quantity, profit, true);
        addTransactionNode(worked);

        int[] ids = findDish(dishName).getDish().getStockID();
            for(int i = 0; i < ids.length; i++){ 
                String ingName = findStockNode(ids[i]).getIngredient().getName();
                updateStock(ingName, ids[i], -quantity);
            }
    }

    private void unsuccessfulTrans(String dishName, int quantity){ 
        TransactionData failed = new TransactionData("order", dishName, quantity, 0.00, false);
        addTransactionNode(failed);

        MenuNode start = findDish(dishName); 
        MenuNode ptr = findDish(dishName).getNextMenuNode();

        while(ptr != null){ 
            dishName = ptr.getDish().getName();
            Boolean check = checkDishAvailability(dishName, quantity);
            if(check == true){ 
                successfulTrans(dishName, quantity);
                return;
                ...
                TransactionData doubleFail = new TransactionData("order", dishName, quantity, 0.00, false);
                addTransactionNode(doubleFail);
            }
        } 
        if(ptr == null){ 
    ...
    public void donation (String ingredientName, int quantity){
        ...
        if ((profit > 50) && (stock >= quantity)) { 
            ...
            findStockNode(ingredientName).getIngredient().updateStockLevel(-quantity);
        } else { 
            TransactionData failed = new TransactionData("donation", ingredientName, quantity, 0, false);
            addTransactionNode(failed);
        }
    ...
    public void restock (String ingredientName, int quantity){
        ...
        if(profit >= totalCost) { 
            TransactionData restock = new TransactionData("restock", ingredientName, quantity, -totalCost, true);
            addTransactionNode(restock);
            findStockNode(ingredientName).getIngredient().updateStockLevel(quantity);
        } else { 
            TransactionData failed = new TransactionData("restock", ingredientName, quantity, 0, false);
            addTransactionNode(failed);
        }
    }
