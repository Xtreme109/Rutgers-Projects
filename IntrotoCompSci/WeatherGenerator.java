...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        for (int y = 0; y < drywet.length; y++) {
            for (int x = 0; x < drywet[y].length; x++) {
                if (longitude == drywet[y][x]) {
                    if (latitude == drywet[y][x+1]) {
                        for (int k = 0; k < 12; k++) {
                            drywetProbability[k] = drywet[y][k+2];
                        }
                    }
                }
            }
        ...
        for (int y = 0; y < wetwet.length; y++) {
            for (int x = 0; x < wetwet[y].length; x++) {
                if (longitude == wetwet[y][x]) {
                    if (latitude == wetwet[y][x+1]) {
                        for (int k = 0; k < 12; k++) {
                            wetwetProbability[k] = wetwet[y][k+2];
                        }
                    }
                }
            }
            ...
            if (i == 0) {
                ...
                if (forecast[i-1] == 2) {
                    if (random >= drywetProbability) {
                        forecast[i] = 2;
                    } else if (random < drywetProbability) {
                        forecast[i] = 1;
                    }
                } else if (forecast[i-1] == 1) {
                    if (random >= wetwetProbability) {
                        forecast[i] = 2;
                    } else if (random < wetwetProbability) {
                        forecast[i] = 1;
                    }
                } 
            }
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        for (int i = 0; i < 12; i++) {
            if (i==month) {
                ...
                drywetProb = drywetProbability[i];
                wetwetProb = wetwetProbability[i];
            }
        }

        int[] forecast = forecastGenerator(drywetProb,wetwetProb,numberOfDays); //make for loops to find the probabilities 


        /*
        for (int i = 0; i < numberOfDays; i++) {  
            System.out.println("Day " + i + ": " + forecast[i]);
        }
        */
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for (int i = 0; i < forecast.length; i++) {
            ...
            /* System.out.println(i + "/" + forecast.length); */
            
            if (mode == forecast[i]) {
                currentstreak++;
            /* System.out.println("currentstreak: " + currentstreak); */
            } else if (mode != forecast[i]) {
    ...
    public static void main (String[] args) {
        ...
        // System.out.println(Arrays.toString(forecastGenerator(.47, .23, 7)));

        StdOut.println("The bestWeekToTravel starts on:" + bestWeek);

        for (int i = 0; i < forecast.length; i++) {
        // // // This is the ternary operator. (conditional) ? executed if true :
        // executed
        // // if
        // // // false
